...
from app.radar.comparacao import avaliar_baseline, com_chaves, data_movimentacao_recente
...
class ExecucaoRadarMemoria:
    ...
    total_nao_localizado: int = 0
    total_numero_invalido: int = 0
    total_pagina_intermediaria: int = 0
...
def _incrementar(execucao: ExecucaoRadarMemoria, resultado: ResultadoConsulta) -> None:
    ...
    if resultado.status == "sucesso":
        ...
        if not resultado.tem_movimentacao_nova:
            execucao.total_sem_movimentacao += 1
        ...
        execucao.total_nao_localizado += 1
    elif resultado.status == "numero_invalido":
        execucao.total_numero_invalido += 1
    elif resultado.status == "pagina_intermediaria":
        execucao.total_pagina_intermediaria += 1
    if resultado.tem_movimentacao_nova:
        execucao.total_com_movimentacao_nova += 1


def registrar_resultado(execucao: ExecucaoRadarMemoria, resultado: ResultadoConsulta) -> None:
...
def consultar_processo_monitorado(
    ...
    *,
    ...
    elif resultado.status in {"timeout", "captcha_timeout", "erro", "senha_necessaria", "pagina_intermediaria"}:
        processo.ultima_consulta_inconclusiva = True
...
def executar(
    ...
    *,
    ...
    execucao = ExecucaoRadarMemoria(origem=origem, usuario_id=usuario_id, total_previstos=len(processos))
    for processo in processos:
        resultado = consultar_processo_monitorado(processo, vault=vault, scrapers=scrapers)
