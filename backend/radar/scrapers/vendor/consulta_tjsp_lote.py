...
PORTA_PADRAO_DRISSIONPAGE = 9222
HOST_DEBUG_CHROME = "127.0.0.1"
VAR_CAMINHO_CHROME = "TJSP_CHROME_PATH"
VAR_PERFIL_CHROME = "TJSP_CHROME_USER_DATA_DIR"
VAR_PROFILE_DIRECTORY_CHROME = "TJSP_CHROME_PROFILE_DIRECTORY"
PASTA_NAVEGADOR = Path(".drissionpage") / "tjsp"
PERFIL_CHROME_PADRAO = PASTA_NAVEGADOR / "perfil"

# True = roda sem abrir janela. False = abre o Chrome visível.
...
def escolher_porta_livre(host: str = HOST_DEBUG_CHROME) -> int:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.bind((host, 0))
        return int(sock.getsockname()[1])


def _normalizar_executavel_chrome(caminho: str | Path) -> Path:
    candidato = Path(caminho).expanduser()
    if candidato.suffix == ".app":
        return candidato / "Contents" / "MacOS" / candidato.stem
    return candidato


def resolver_caminho_chrome() -> Path:
    candidatos: list[Path] = []
    for var_name in (VAR_CAMINHO_CHROME, "GOOGLE_CHROME_PATH", "CHROME_PATH"):
        env_value = os.environ.get(var_name)
        if env_value:
            found = shutil.which(env_value)
            candidatos.append(
                _normalizar_executavel_chrome(found or env_value)
            )

    candidatos.extend(
        _normalizar_executavel_chrome(path)
        for path in (
    ...
    for command in (
        ...
        if found:
            candidatos.append(Path(found))

    verificados: list[str] = []
    for candidato in candidatos:
        verificados.append(str(candidato))
        if candidato.exists() and candidato.is_file():
            return candidato.resolve()

    raise FileNotFoundError(
        "Google Chrome não encontrado. Configure "
        f"{VAR_CAMINHO_CHROME} com o caminho do executável. "
        f"Candidatos verificados: {verificados}"
    )


def perfil_chrome_configurado() -> Path:
    raw_path = os.environ.get(VAR_PERFIL_CHROME)
    path = Path(raw_path) if raw_path else PERFIL_CHROME_PADRAO
    return path.expanduser().resolve()


def profile_directory_chrome() -> str:
    return os.environ.get(VAR_PROFILE_DIRECTORY_CHROME, "Default").strip() or "Default"


def pids_usando_perfil(user_data_path: Path) -> list[int]:
    profile = str(user_data_path.expanduser().resolve())
    markers = (
...
def preparar_perfil_navegador() -> tuple[Path, str, list[int], str]:
    profile_path = perfil_chrome_configurado()
    profile_kind = "persistente"
    pids = pids_usando_perfil(profile_path)

    if pids:
        profile_kind = "isolado_por_conflito"
        profile_path = (
            PASTA_NAVEGADOR / f"perfil_execucao_{os.getpid()}"
        ).expanduser().resolve()
        logger.warning(
            "Perfil persistente já está em uso por PIDs %s. "
            "Usando perfil isolado nesta execução para evitar conflito: %s",
            pids,
            profile_path,
        )

    profile_path.mkdir(parents=True, exist_ok=True)
    return profile_path, profile_directory_chrome(), pids, profile_kind
...
def preencher_senha_processo(tab: Any, senha: str) -> bool:
    selectors = (
        'input[type="password"]',
    ...
    for selector in selectors:
        try:
            field = tab.ele(selector, timeout=0.4)
        except TypeError:
    ...
    for selector in ('button[type="submit"]', 'input[type="submit"]', "#sbmNovo"):
        try:
            button = tab.ele(selector, timeout=0.4)
        except TypeError:
        ...
        if button and hasattr(button, "click"):
            try:
                button.wait.clickable(timeout=3)
            except Exception:
                pass
            button.click()
            return True
...
def executar_tentativa(
    ...
    try:
        ...
        if result.get("status") == "senha_necessaria" and item.get("senha"):
            logger.info("Processo exige senha; preenchendo senha cadastrada no cofre.")
            if preencher_senha_processo(tab, item["senha"]):
                pausa(PAUSA_ANTES_DE_CONSULTAR)
                result = aguardar_resultado(tab, portal, item["numero_processo"])
            else:
                result = {
                    "status": "senha_necessaria",
                    "mensagem": "Pagina solicitou senha, mas o campo de senha nao foi localizado.",
                    "quantidade_movimentacoes": 0,
                    "movimentacoes": [],
                    "url_resultado": getattr(tab, "url", None),
                }
...
def criar_navegador() -> ChromiumPage:
    if porta_tcp_em_uso(HOST_DEBUG_CHROME, PORTA_PADRAO_DRISSIONPAGE):
        logger.info(
            "Porta padrão do DrissionPage %s está ocupada e será ignorada.",
            PORTA_PADRAO_DRISSIONPAGE,
        )
    else:
        logger.info(
            "Porta padrão do DrissionPage %s não será usada; "
            "uma porta livre será escolhida automaticamente.",
            PORTA_PADRAO_DRISSIONPAGE,
        )

    browser_path = resolver_caminho_chrome()
    local_port = escolher_porta_livre()
    user_data_path, profile_directory, profile_pids, profile_kind = (
        preparar_perfil_navegador()
    )
    ...
    options.set_user(profile_directory)
    options.headless(NAVEGADOR_HEADLESS)
    ...
    logger.info("Caminho do navegador: %s", browser_path)
    logger.info("Modo headless: %s", NAVEGADOR_HEADLESS)
    logger.info("Porta escolhida para depuração remota: %s", local_port)
    logger.info(
        "Perfil utilizado: user-data-dir=%s | profile-directory=%s | tipo=%s",
        user_data_path,
        profile_directory,
        profile_kind,
    )
    if profile_pids:
        logger.warning(
            "Conflito de perfil detectado antes da inicialização: PIDs %s",
            profile_pids,
        )
    logger.info(
        "Tentativa de inicialização: DrissionPage iniciará o Chrome "
        "automaticamente em %s:%s.",
        HOST_DEBUG_CHROME,
        local_port,
    )
    ...
    try:
        ...
        logger.error(
            "Falha ao criar conexão do DrissionPage | endereço=%s:%s | "
            "porta_em_uso=%s | perfil_em_uso_pids=%s | motivo=%s",
            HOST_DEBUG_CHROME,
            local_port,
            porta_tcp_em_uso(HOST_DEBUG_CHROME, local_port),
            pids_usando_perfil(user_data_path),
            exc,
            exc_info=True,
        )
        raise
        ...
        logger.exception(
            "Falha inesperada ao iniciar navegador | caminho=%s | porta=%s | "
            "perfil=%s | motivo=%s",
            browser_path,
            local_port,
            user_data_path,
    ...
    logger.info(
        "ChromiumPage criado com sucesso | address=%s | versão=%s | pid=%s",
        getattr(page, "address", None),
        getattr(page, "browser_version", None),
